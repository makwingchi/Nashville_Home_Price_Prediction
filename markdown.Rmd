...
```{r distribution of saleprice, message = FALSE, out.width=c('50%', '50%'), fig.show='hold'}
ggplot(vars_train) +
...
```{r correlation matrix, fig.height = 6, fig.width = 6}
...
```{r homesale price map, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
...
```{r public art collections, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
  geom_sf(data = baseMap, inherit.aes = FALSE, color='grey21', size=0.8) +
  geom_point(data=vars_train, 
...
```{r total finished area less an adjustment, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
  geom_sf(data = baseMap, inherit.aes = FALSE, color='grey21', size=0.8) +
  geom_point(data=vars_train, 
...
```{r proportion of economically disabled students, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
  geom_sf(data = baseMap, inherit.aes = FALSE, color='grey21', size=0.8) +
  geom_point(data=vars_train, 
...
```{r variable distribution boxplots, fig.height = 12, fig.width = 8}
...
```{r predicted prices as a function of observed prices}
ggplot() + 
...
```{r histogram of residuals (absolute values), message=FALSE}
ggplot() +
...
```{r predicted prices for home sales, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
  geom_sf(data = baseMap, inherit.aes = FALSE, color='grey21', size=0.8) +
...
```{r predicted prices as a function of observed prices (25 pct), message=FALSE}
ggplot() + 
...
```{r residuals for 25 pct randomly selected test set, message=FALSE, fig.height = 6, fig.width = 7}
ggmap(map3) +
  geom_sf(data = baseMap, inherit.aes = FALSE, color='grey21', size=0.8) +
...
```{r histogram of 100-fold cv R^2, message=FALSE, out.width=c('50%', '50%'), fig.show='hold'}
...
```{r MAPE by zipcode map, message=FALSE, fig.height = 5, fig.width = 7, fig.align = 'center'}
ggmap(map3) + 
...
```{r MAPE and Average Sale Price by Zipcode}
...
```{r MAPE as a function of mean price by zipcode, message=FALSE}
...
```{r table of MAPE and MAE for each three holdout zipcodes, message=FALSE}
...
```{r scatterplot of predicted vs. observed for the three neighborhoods, message=FALSE}
ggplot(Compare_Neighborhoods) +
  geom_point(aes(observedSales, predictedSales)) + 
  stat_smooth(aes(observedSales, predictedSales), method = 'lm', se = F, color = '#b8d1e3') +
  stat_smooth(aes(observedSales, observedSales), method = 'lm', se = F, color = '#fa725a') +
